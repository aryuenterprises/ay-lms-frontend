...
const DashboardDefault = () => {
  ...
  const [viewMode, setViewMode] = useState('daily');
  const mergedDailyData = dailyRevenue.map((rev) => {
    const reg = dailyRegistrations.find((r) => new Date(r.day).toDateString() === new Date(rev.day).toDateString());
  ...
  const latestMonth = overallRevenueData.length > 0 ? overallRevenueData[overallRevenueData.length - 1] : null;

  const previousMonth = overallRevenueData.length > 1 ? overallRevenueData[overallRevenueData.length - 2] : null;

  const growth =
    previousMonth && previousMonth.total > 0 ? (((latestMonth.total - previousMonth.total) / previousMonth.total) * 100).toFixed(1) : null;

  const mergedMonthlyData = monthlyRevenue.map((rev) => {
    const reg = monthlyRegistrations.find((r) => new Date(r.month).getMonth() === new Date(rev.month).getMonth());
                ...
                >
                  <Stack direction="row" justifyContent="space-between" alignItems="center">
                    <Box>
                      ...
                      <Typography variant="h4" sx={{ fontWeight: 700, color: '#7a1628' }}>
                        {data?.active_batches}
                      </Typography>
                    </Box>
            ...
            <>
              ...
              <Grid item xs={12} md={7} lg={6}>
                <Box
                  sx={{
                    ...
                    p: 4,
                    borderRadius: 4,
                    background: 'linear-gradient(135deg, #fff6f6 0%, #ffffff 40%, #fff1f1 100%)',
                    border: '1px solid #f3d6d6',
                    position: 'relative',
                    overflow: 'hidden'
                  }}
                >
                    ...
                    >
                      <Button
                        ...
                        sx={{
                          ...
                          fontWeight: 600,
                          textTransform: 'none',
                          backgroundColor: viewMode === 'daily' ? '#bd1a25' : 'transparent',
                          color: viewMode === 'daily' ? '#ffffff' : '#c1121f',
                          '&:hover': {
                            backgroundColor: viewMode === 'daily' ? '#9b0e19' : 'rgba(193,18,31,0.08)'
                          }
                        }}
                      >
                        Daily
                      ...
                      <Button
                        ...
                        sx={{
                          ...
                          fontWeight: 600,
                          textTransform: 'none',
                          backgroundColor: viewMode === 'monthly' ? '#c1121f' : 'transparent',
                          color: viewMode === 'monthly' ? '#ffffff' : '#c1121f',
                          '&:hover': {
                            backgroundColor: viewMode === 'monthly' ? '#9b0e19' : 'rgba(193,18,31,0.08)'
                          }
                        }}
                      >
                        Monthly
                  ...
                  {/* Chart */}
                  <ResponsiveContainer width="100%" height={360}>
                    <LineChart data={viewMode === 'daily' ? mergedDailyData : mergedMonthlyData}>
                      <defs>
                        <linearGradient id="revenueFill" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%" stopColor="#ac9d19" stopOpacity={0.4} />
                          <stop offset="95%" stopColor="#e0d318" stopOpacity={0} />
                        </linearGradient>

                        <linearGradient id="registrationFill" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%" stopColor="#fbf271" stopOpacity={0.4} />
                          <stop offset="95%" stopColor="#b8ac09" stopOpacity={0} />
                        </linearGradient>
                      </defs>

                      <CartesianGrid strokeDasharray="4 4" vertical={false} stroke="#f3d6d6" />
              ...
              <Grid item xs={12} md={5} lg={6}>
                ...
                >
                  ...
                  {/* Main Revenue */}
                  <Typography variant="h3" sx={{ fontWeight: 800, color: '#7a1628' }}>
                    ₹ {latestMonth?.total?.toLocaleString()}
                  </Typography>
                  ...
                  <Box
                    sx={{
                      ...
                      py: 0.5,
                      borderRadius: 2,
                      backgroundColor: growth > 0 ? 'rgba(16,185,129,0.1)' : 'rgba(220,38,38,0.1)',
                      color: growth > 0 ? '#16a34a' : '#dc2626',
                      fontWeight: 600,
                      mb: 4
                    }}
                    ...
                    {overallRevenueData.map((item, index) => (
                      <Box key={index} sx={{ mb: 2 }}>
                        <Stack direction="row" justifyContent="space-between" sx={{ mb: 0.5 }}>
                          ...
                          <Typography variant="body2" sx={{ fontWeight: 600, color: '#7a1628' }}>
                            ₹ {item.total.toLocaleString()}
                          </Typography>
                        </Stack>
                        ...
                        >
                          <Box
                            sx={{
                              height: '100%',
                              width: `${(item.total / Math.max(...overallRevenueData.map((r) => r.total))) * 100}%`,
                              background: 'linear-gradient(90deg, #7a1628, #c1121f)'
                            }}
                          />
                        </Box>
                      </Box>
                  ...
                  <TableBody>
                    {featuredCourses.map((course, index) => (
                      <TableRow key={course.course_id} hover>
                        ...
                        {/* Category */}
                        <TableCell>{course.course_category}</TableCell>
                        ...
                        <TableCell align="center">
                          <Button variant="contained" size="small" onClick={() => handleEnrollNow(course)}>
                            Enroll Now
                          </Button>
                        </TableCell>
                      </TableRow>
