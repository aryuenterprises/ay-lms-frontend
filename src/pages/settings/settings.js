...
const Settings = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (values.general_logo instanceof File) {
          formData.append('general_logo', values.general_logo);
        }
        if (values.secondary_logo instanceof File) {
          formData.append('secondary_logo', values.secondary_logo);
        }
  ...
  const handleFileChange = (event, fieldName) => {
    ...
    if (file) {
      // Pre-allocate space by setting a temporary placeholder
      const previewUrl = URL.createObjectURL(file);
      formik.setFieldValue(fieldName, file);
      formik.setFieldValue(`${fieldName}_url`, previewUrl);
  ...
  const getImageSource = (fieldName) => {
    ...
    const previewUrl = formik.values[`${fieldName}_url`];
        ...
        >
          <img
            ...
            onError={(e) => {
              ...
              if (container) {
                container.innerHTML = '<Typography variant="body2" color="text.secondary">Error loading image</Typography>';
              }
            }}
          />
        </Box>
