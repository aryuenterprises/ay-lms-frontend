...
const WebinarList = () => {
  ...
  const handleEdit = (webinar) => {
    ...
    const updatingItemformat = webinar?.tools.map((value) => {
      return {
        title: value?.tools_title,
        file: value?.tools_image,
        preview: value?.image_url,
        id: value?.id,
        istextModify: false,
        isFileModify: false,
        isDelete: value?.is_deleted,
        oldData: true
      };
    });
  ...
  const handleWebinarstart = useCallback(
    async (webinar) => {
      try {
        ...
        await axiosInstance.post(`/api/webinar/${webinar.uuid}/session/start/`);

        // Optional: refresh list AFTER returning from session
        fetchData();
      } catch (error) {
  ...
  const handleWebinarend = useCallback(
    async (webinar) => {
      try {
        // Call session end API (backend marks FINISHED)
        await axiosInstance.post(`/api/webinar/${webinar.uuid}/session/end/`);

        // Call attendance sync API
    ...
    onSubmit: async (values, { resetForm, setSubmitting }) => {
      try {
        ...
        const appendIfExists = (key, value) => {
          if (value !== undefined && value !== null && value !== '' && !(value instanceof File && value.size === 0)) {
        ...
        let deleteIndex = 0;
        {
          !editToolsOpen
            ? items.forEach((tool, index) => {
                payload.append(`tools[${index}][tools_title]`, tool.title);

                payload.append(`tools[${index}][tools_image]`, tool.file);
              })
            : editTools.map((value) => {
                if (value.isDelete) {
                  if (value.oldData) {
                    payload.append(`tools[${deleteIndex}][id]`, value.id);
                    payload.append(`tools[${deleteIndex}][is_deleted]`, true);
                    deleteIndex++;
                  }
                } else {
                  if (value?.oldData) {
                    if (value.isDelete) {
                      ...
                      if (value.istextModify) {
                        payload.append(`tools[${deleteIndex}][id]`, value.id);
                        payload.append(`tools[${deleteIndex}][tools_title]`, value.title);
                        deleteIndex++;
                      }

                      if (value.isFileModify) {
                        payload.append(`tools[${deleteIndex}][id]`, value.id);
                        payload.append(`tools[${deleteIndex}][tools_image]`, value.file);
                        deleteIndex++;
                      }
                    }
                  } else {
  ...
  const columns = [
    ...
    {
      name: 'Date & Time',
      cell: (row) => <div style={{ whiteSpace: 'normal', lineHeight: '1.4' }}>{formatDateTime(row.scheduled_start)}</div>,
      sortable: true,
      wrap: true
    },
    {
      ...
      cell: (row) => (
        <Box sx={{ display: 'flex', gap: 1 }}>
          {row.webinar_status === true ? (
            ...
            <Button variant="outlined" color="error" size="small" onClick={() => handleStatusChange('SHEDULED', row)}>
              No
            </Button>
          ) : (
            <Button variant="outlined" color="error" size="small" onClick={() => handleStatusChange('DRAFT', row)}>
              Inactivate
            </Button>
          )}
        </Box>
      ...
      cell: (row) => (
        <Box sx={{ display: 'flex', gap: 1 }}>
          {row.status === 'COMPLETED' ? (
            ...
            <Button variant="contained" color="error" size="small" onClick={() => handleWebinarstart(row)}>
              Go Live
            </Button>
          )}
        </Box>
                          ...
                          <Stack spacing={1}>
                            ...
                            {imagePreviewweb && (
                              <Grid item xs={12} md={6} sx={{ p: 1 }}>
                                ...
                                <img src={imagePreviewweb} alt="webinar Preview" width={120} style={{ borderRadius: 8, marginTop: 8 }} />
                              </Grid>
                            )}
                          </Stack>
                        </Grid>
                        <Grid item xs={12} md={6} sx={{ pt: 1 }}>
                          <Stack spacing={1}>
                            <Grid item xs={12} md={6} sx={{ p: 2 }}>
                              <Button startIcon={<YouTubeIcon />} variant="outlined" onClick={() => setOpenVideoDialog(true)}>
                                Add YouTube Video
                              </Button>
                            </Grid>
                            {videoUrl && (
                          ...
                          <Stack spacing={1}>
                            ...
                            <Grid display="flex" gap={2}>
                              <ButtonBase
                                ...
                                sx={{
                                  border: '2px solid',
                                  borderColor: formik.values.webinarType === 'Free' ? 'primary.main' : 'grey.400',
                                  bgcolor: formik.values.webinarType === 'Free' ? 'primary.light' : 'grey.100',
                                  px: 3,
                                  py: 1.5,
                                  borderRadius: 2
                                }}
                              ...
                              <ButtonBase
                                ...
                                sx={{
                                  border: '2px solid',
                                  borderColor: formik.values.webinarType === 'Paid' ? 'success.main' : 'grey.400',
                                  bgcolor: formik.values.webinarType === 'Paid' ? 'success.light' : 'grey.100',
                                  px: 3,
                                  py: 1.5,
                                  borderRadius: 2
                                }}
                          ...
                          <Paper sx={{ p: 4, display: 'flex', gap: 2, alignItems: 'center' }}>
                            ...
                            <Grid item xs={12} md={6} sx={{ pt: 3 }}>
                              <Stack spacing={2}>
                                ...
                                {imagePreview && (
                                  <img src={imagePreview} alt="Meta Preview" width={120} style={{ borderRadius: 8, marginTop: 8 }} />
                                )}
                              </Stack>
                            </Grid>
                      ...
                      <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
                        ...
                        <Grid item xs={12} md={4} sx={{ pt: 4 }}>
                          <Stack spacing={2}>
                            ...
                            <Select name="status" label="status" displayEmpty value={status} onChange={(e) => setStatus(e.target.value)}>
                              <MenuItem value="" disabled>
                            ...
                            {items &&
                              ...
                              items.map((item, index) => (
                                <Paper key={index} elevation={2} sx={{ p: 2, display: 'flex', gap: 2, alignItems: 'center' }}>
                                  ...
                                  {item.preview && (
                                    <img src={item.preview} width={48} height={48} style={{ borderRadius: 6 }} alt="preview" />
                                  )}

                                  <Button variant="contained" size="small" type="remove" onClick={() => handleRemove(index)}>
                                    Remove
                                  </Button>
                                </Paper>
                              ))}
                                  ...
                                  !item.isDelete && (
                                    <Paper key={item.id} elevation={2} sx={{ p: 2, display: 'flex', gap: 2, alignItems: 'center' }}>
                                      <TextField
                                        ...
                                        label="Title"
                                        value={item.title}
                                        onChange={(e) => handleEditTitleChange(index, e.target.value)}
                                      />
                                      ...
                                      <Button variant="contained" size="small" type="remove" onClick={() => handleEditRemove(index)}>
                                        Remove
                                      </Button>
                                    </Paper>
                                  )
                          ...
                          {faqs &&
                            ...
                            faqs.map((faqs, index) => (
                              <Paper key={index} elevation={2} sx={{ p: 2, display: 'flex', gap: 2, alignItems: 'center' }}>
                                ...
                                <Button variant="contained" size="small" type="remove" onClick={() => handleFaqremove(index)}>
                                  Remove
                                </Button>
                              </Paper>
                            ))}
              ...
              <Grid container>
                ...
                <DialogTitle className="dialogTitle">
                  <Grid container spacing={2} sx={{ p: 3 }}>
                    ...
                    <Grid item xs={12} md={4} sx={{ pt: 4 }}>
                      <Stack spacing={2}>
                        <Typography fontWeight={400}>WhatsApp Link</Typography>

                        {viewDetails?.waba_link ? (
                          <Box
                            component="a"
                            href={viewDetails.waba_link}
                            target="_blank"
                            rel="noopener noreferrer"
                            sx={{
                              p: 1.5,
                    ...
                    <Grid container spacing={3}>
                      {viewDetails &&
                        ...
                        viewDetails?.tools.map((item, index) => {
                          return (
