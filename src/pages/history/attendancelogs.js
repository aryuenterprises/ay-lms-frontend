...
const filteredData = useMemo(() => {
  const filtered = data.filter((item) => {
    ...
    // course filter
    if (selectedCourse && item.course_id !== selectedCourse.course_id) {
      return false;
    }

    // batch filter
    if (selectedBatch && item.batch_id !== selectedBatch.batch_id) {
      return false;
    }

    // user type filter
    if (filterUserType && item.user_type !== filterUserType) {
      return false;
    }
  ...
  // ✅ ADD DYNAMIC SNO HERE
  return filtered.map((item, index) => ({
    ...item,
    sno: index + 1
  }));

}, [data, filterText, filterName, selectedCourse, selectedBatch, filterUserType, startDate, endDate]);
  ...
  const columns =
    userType === 'tutor'
      ...
      : [
          ...
          // {
          //   name:"search by name",
          //   selector:(row)=>Capitalise(now.user_type)||'-',
          //   sortable:true
          // },
          {
            name: 'Batch',
           ...
           <TextField
            ...
            InputProps={{
              ...
              endAdornment: (
                <InputAdornment position="end">
                  {filterName && (
                    <IconButton onClick={handleClearName} edge="end" size="small">
                      <CloseSquare size={20} />
                    </IconButton>
                  )}
                </InputAdornment>
