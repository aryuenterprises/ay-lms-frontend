...
const BatchManagement = () => {
  ...
  console.log('filteredBatches', filteredBatches);
  return (
    <MainCard sx={{ py: 4, border: '1px solid #e9ddf0', boxShadow: '0 15px 40px #e9ddf0' }}>
      <Grid container justifyContent="space-between" alignItems="center" my={3} spacing={2}>
        <Grid item xs={12}>
          <Stack direction={{ xs: 'column', sm: 'row' }} alignItems={{ xs: 'stretch', sm: 'center' }} spacing={2} width="100%">
            {/* Filters Container - Wraps to next row */}
                    ...
                    {canUpdate ? (
                      <Tooltip title="Status">
                        <Switch
                          ...
                          color={batch?.status ? 'success' : 'error'}
                          onClick={(e) => e.stopPropagation()}
                          onChange={(e) => {
                            e.stopPropagation();
                            handleStatusChange(batch, !batch?.status);
                          }}
                        />
                      </Tooltip>
                    ) : (
                      <Chip label={batch?.status ? 'Active' : 'Inactive'} color={batch?.status ? 'success' : 'error'} size="small" />
                    )}
                  </TableCell>

                  {/* Start Date */}
                  <TableCell>{formatDateTime(batch?.start_date, { includeTime: false })}</TableCell>
