...
const StudentTable = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const dataWithSerial = data?.map((item) => ({ ...item }));
  ...
  const validationSchema = Yup.object().shape({
    ...
    contact_no: Yup.string()
      ...
      .max(16, "phone number cannot exceed 15")
    ...
    // ✅ FIXED: year_of_study validation kept, field now exists in the form
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      // ✅ FIXED: college_name no longer shows "null" string
      college_name: (currentStudent?.college_student?.college_name === 'null' ? '' : currentStudent?.college_student?.college_name) || '',
  ...
  const { filteredCategories, filteredCourses, filteredBatches, filteredCompanies, filteredStudents } = useMemo(() => {
    ...
    return { filteredCategories, filteredCourses, filteredBatches, filteredCompanies, filteredStudents };
  }, [data, categories, courses, batches, companies, selectedCategory, selectedCourse, selectedBatch, selectedCompany, statusFilter, typeFilter, filterText]);
  ...
  const subHeaderComponentMemo = useMemo(() => {
    ...
    const handleStatusClear = () => { setStatusFilter('all'); };
    const handleTypeClear = () => { setTypeFilter('all'); };
      ...
      <Grid container justifyContent="space-between" alignItems="center" my={3} spacing={2}>
        <Grid item xs={12}>
          <Stack direction={{ xs: 'column', sm: 'row' }} alignItems={{ xs: 'stretch', sm: 'center' }} spacing={2} width="100%">
            <Box sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 2, width: { xs: '100%', sm: 'auto' }, '& > *': { minWidth: { xs: 'calc(50% - 8px)', sm: 200 }, flexGrow: 1 } }}>
              <TextField
              ...
              {(userType === 'admin' || userType === 'super_admin') && (
                <>
                  <Autocomplete
                    ...
                    sx={{ minWidth: { xs: '100%', sm: 180, md: 200 }, flex: '1 1 auto' }}
                    size="small"
                    renderInput={(params) => (<TextField {...params} placeholder="Filter by category..." InputProps={{ ...params.InputProps }} />)}
                    filterOptions={(options = [], state) => options.filter((option) => option.category_name?.toLowerCase().includes(state.inputValue.toLowerCase()))}
                    isOptionEqualToValue={(option, value) => option.category_id === value.category_id}
                    renderOption={(props, option) => (<li {...props} key={option.category_id}>{option.category_name}</li>)}
                  />
                  <Autocomplete
                    id="course_id"
                    ...
                    filterOptions={(options = [], state) => options.filter((option) => option.course_name?.toLowerCase().includes(state.inputValue.toLowerCase()))}
                    isOptionEqualToValue={(option, value) => option.course_id === value.course_id}
                    renderOption={(props, option) => (<li {...props} key={option.course_id}>{option.course_name}</li>)}
                  />
                  <Autocomplete
                    id="batch_id"
                    ...
                    value={selectedBatch}
                    onChange={(event, newValue) => { handleBatchSelect(newValue); }}
                    size="small"
                    sx={{ minWidth: { xs: '100%', sm: 180, md: 200 }, flex: '1 1 auto' }}
                    renderInput={(params) => (<TextField {...params} placeholder="Filter by batch..." InputProps={{ ...params.InputProps }} />)}
                    ...
                    renderOption={(props, option) => (<li {...props} key={option.batch_id}>{option.title}</li>)}
                  />
                  <Autocomplete
                    id="company_id"
                    ...
                    size="small"
                    renderInput={(params) => (<TextField {...params} placeholder="Filter by organization..." InputProps={{ ...params.InputProps }} />)}
                    filterOptions={(options = [], state) => options.filter((option) => option.company_name?.toLowerCase().includes(state.inputValue.toLowerCase()))}
                    isOptionEqualToValue={(option, value) => option.company_id === value.company_id}
                    renderOption={(props, option) => (<li {...props} key={option.company_id}>{option.company_name}</li>)}
                    sx={{ minWidth: { xs: '100%', sm: 180, md: 200 }, flex: '1 1 auto' }}
                  />
                  <FormControl size="small" sx={{ minWidth: { xs: '100%', sm: 180 }, flex: { xs: '1 1 100%', sm: '0 1 auto' }, maxWidth: { sm: 250 } }}>
                    <Select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} displayEmpty
                      endAdornment={typeFilter !== 'all' && (<InputAdornment position="end" sx={{ mr: 3 }}><IconButton onClick={handleTypeClear} edge="end" size="small"><CloseSquare size={16} /></IconButton></InputAdornment>)}>
              ...
              <FormControl size="small" sx={{ minWidth: { xs: '100%', sm: 180 }, flex: { xs: '1 1 100%', sm: '0 1 auto' }, maxWidth: { sm: 250 } }}>
                <Select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} displayEmpty
                  endAdornment={statusFilter !== 'all' && (<InputAdornment position="end" sx={{ mr: 3 }}><IconButton onClick={handleStatusClear} edge="end" size="small"><CloseSquare size={16} /></IconButton></InputAdornment>)}>
        ...
        <Grid item xs={12}>
          <Stack spacing={2} direction="row" justifyContent="flex-end">
            {(userType === 'admin' || userType === 'super_admin') && canCreate && (
              <Button color="success" variant="contained" startIcon={<UserAdd />} onClick={handleOpen} sx={{ width: { xs: '100%', sm: 'auto' } }}>
                Add
              </Button>
            )}
          </Stack>
  ...
  }, [filterText, resetPaginationToggle, userType, selectedCompany, statusFilter, typeFilter, canCreate, filteredCategories, filteredCourses, filteredBatches, filteredCompanies, selectedCategory, selectedCourse, selectedBatch]);

  if (error) {
    return (
      <MainCard sx={{ borderRadius: 2 }}>
        <Box p={3} color="error.main">Error: {error}</Box>
      </MainCard>
    );
  }
  ...
  return (
    <MainCard sx={{ borderRadius: 2 }}>
      ...
      >
        ...
        <DialogContent>
          <form onSubmit={formik.handleSubmit}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField fullWidth id="first_name" placeholder="First Name" name="first_name"
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField fullWidth id="last_name" placeholder="Last Name" name="last_name"
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField fullWidth id="username" placeholder="User Name" name="username"
                    value={formik.values.username} onChange={formik.handleChange} onBlur={formik.handleBlur}
                    error={formik.touched.username && Boolean(formik.errors.username)}
                    helperText={formik.touched.username && formik.errors.username} autoComplete="new-username" />
                </Stack>
              </Grid>
              {/*Password*/}
              {!currentStudent && (
                <Grid item xs={12} md={6}>
                  <Stack sx={{ mt: 2, gap: 1 }}>
                    ...
                    <TextField fullWidth id="password" placeholder="Password" name="password"
                      type={showPassword ? 'text' : 'password'}
                      value={formik.values.password} onChange={formik.handleChange} onBlur={formik.handleBlur}
                      error={formik.touched.password && Boolean(formik.errors.password)}
                      helperText={formik.touched.password && formik.errors.password} autoComplete="new-password"
                      InputProps={{ endAdornment: (<InputAdornment position="end"><IconButton onClick={() => setShowPassword(!showPassword)} edge="end">{showPassword ? <Eye /> : <EyeSlash />}</IconButton></InputAdornment>) }} />
                  </Stack>
                </Grid>
              )}
              {/*Email Address*/}
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField fullWidth id="email" placeholder="Email Address" name="email" type="email"
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField fullWidth id="contact_no" placeholder="Mobile" name="contact_no" type="number"
                    value={formik.values.contact_no} onChange={formik.handleChange} onBlur={formik.handleBlur}
                ...
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <LocalizationProvider dateAdapter={AdapterDateFns}>
                    <DatePicker
                      ...
                      onChange={(date) => { formik.setFieldValue('dob', date); formik.setFieldTouched('dob', true, false); }}
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField fullWidth id="current_address" placeholder="Current address" name="current_address"
                    value={formik.values.current_address} onChange={formik.handleChange} onBlur={formik.handleBlur}
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField fullWidth id="permanent_address" placeholder="Permanent address" name="permanent_address"
                    value={formik.values.permanent_address} onChange={formik.handleChange} onBlur={formik.handleBlur}
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField fullWidth id="city" placeholder="City" name="city"
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField fullWidth id="state" placeholder="State" name="state"
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <Autocomplete
                    id="country" options={countries || []}
                    getOptionLabel={(option) => `${option.label} (${option.phone})`}
                    value={countries?.find((c) => c.label === formik.values.country) || null}
                    onChange={(event, newValue) => { formik.setFieldValue('country', newValue?.label || ''); }}
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField select fullWidth name="internship"
                    value={formik.values.internship} onChange={formik.handleChange} onBlur={formik.handleBlur}
                    ...
                    {["Not", 1, 2, 3, 4, 5, 6].map((month) => (
                      <MenuItem key={month} value={month.toString()}>
                        {typeof month === 'number' ? `${month} month${month !== 1 ? 's' : ''}` : month}
                      </MenuItem>
                    ))}
                  </TextField>
                </Stack>
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField fullWidth id="parent_guardian_name" placeholder="Parent (or) Guardian Name" name="parent_guardian_name"
                    value={formik.values.parent_guardian_name} onChange={formik.handleChange} onBlur={formik.handleBlur}
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField fullWidth type="number" id="parent_guardian_phone" placeholder="Parent (or) Guardian Number" name="parent_guardian_phone"
                    value={formik.values.parent_guardian_phone} onChange={formik.handleChange} onBlur={formik.handleBlur}
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField fullWidth id="parent_guardian_occupation" placeholder="Parent (or) Guardian Address" name="parent_guardian_occupation"
                    value={formik.values.parent_guardian_occupation} onChange={formik.handleChange} onBlur={formik.handleBlur}
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField fullWidth id="reference_number" placeholder="Referral Number" name="reference_number"
                    value={formik.values.reference_number} onChange={formik.handleChange} onBlur={formik.handleBlur}
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField fullWidth id="source_type" placeholder="source" name="source_type" type="text"
                    value={formik.values.source_type} onChange={formik.handleChange} onBlur={formik.handleBlur}
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField name="student_type" select fullWidth placeholder="Student Type"
                    value={formik.values.student_type} onChange={formik.handleChange} onBlur={formik.handleBlur}
                ...
                <>
                  <Grid item xs={12} md={6}>
                    <Stack sx={{ mt: 2, gap: 1 }}>
                      ...
                      <TextField fullWidth id="school_name" placeholder="School Name" name="school_name"
                        value={formik.values.school_name} onChange={formik.handleChange} onBlur={formik.handleBlur}
                  ...
                  <Grid item xs={12} md={6}>
                    <Stack sx={{ mt: 2, gap: 1 }}>
                      ...
                      <TextField fullWidth id="school_class" placeholder="Class/Grade" name="school_class"
                        value={formik.values.school_class} onChange={formik.handleChange} onBlur={formik.handleBlur}
                ...
                <>
                  ...
                  <Grid item xs={12} md={6}>
                    <Stack sx={{ mt: 2, gap: 1 }}>
                      ...
                      <TextField fullWidth id="college_name" placeholder="College Name" name="college_name"
                        value={formik.values.college_name} onChange={formik.handleChange} onBlur={formik.handleBlur}
                  ...
                  <Grid item xs={12} md={6}>
                    <Stack sx={{ mt: 2, gap: 1 }}>
                      ...
                      <TextField fullWidth id="degree" placeholder="Degree" name="degree"
                        value={formik.values.degree} onChange={formik.handleChange} onBlur={formik.handleBlur}
                        error={formik.touched.degree && Boolean(formik.errors.degree)}
                        helperText={formik.touched.degree && formik.errors.degree} />
                    </Stack>
                  </Grid>
                  {/* ✅ FIXED: Year of Study field was MISSING — now added */}
                  <Grid item xs={12} md={6}>
                    <Stack sx={{ mt: 2, gap: 1 }}>
                      <FormLabel>Year of Study*</FormLabel>
                      <TextField fullWidth id="year_of_study" placeholder="Year of Study (e.g. 2nd Year)" name="year_of_study"
                        value={formik.values.year_of_study} onChange={formik.handleChange} onBlur={formik.handleBlur}
                    ...
                    <Stack sx={{ mt: 2, gap: 1 }}>
                      ...
                      {formik.values.resume ? (
                        <Stack direction="row" alignItems="center" spacing={2}>
                          <Box display="flex" alignItems="center" justifyContent="space-between" gap={1} sx={{ border: '1px dashed', borderRadius: 1, p: 1 }}>
                            <Typography variant="body2">
                              Current file: {typeof formik.values.resume === 'string' ? formik.values.resume.split('/').pop() : formik.values.resume.name}
                            </Typography>
                          ...
                          <Button variant="outlined" component="label" size="small">
                            Change File
                            <input type="file" hidden id="resume" name="resume"
                              onChange={(event) => { formik.setFieldValue('resume', event.currentTarget.files[0]); }}
                              onBlur={formik.handleBlur} />
                          </Button>
                        </Stack>
                      ) : (
                        <TextField fullWidth type="file" id="resume" name="resume" InputLabelProps={{ shrink: true }}
                          onChange={(event) => { formik.setFieldValue('resume', event.currentTarget.files[0]); }}
                          onBlur={formik.handleBlur}
                ...
                <>
                  <Grid item xs={12} md={6}>
                    <Stack sx={{ mt: 2, gap: 1 }}>
                      ...
                      <TextField fullWidth id="passed_out_year" placeholder="Passed Out Year" name="passed_out_year" type="number"
                        value={formik.values.passed_out_year} onChange={formik.handleChange} onBlur={formik.handleBlur}
                  ...
                  <Grid item xs={12} md={6}>
                    <Stack sx={{ mt: 2, gap: 1 }}>
                      ...
                      <TextField fullWidth id="current_qualification" placeholder="Current Qualification" name="current_qualification"
                        value={formik.values.current_qualification} onChange={formik.handleChange} onBlur={formik.handleBlur}
                  ...
                  <Grid item xs={12} md={6}>
                    <Stack sx={{ mt: 2, gap: 1 }}>
                      ...
                      <TextField fullWidth id="preferred_job_role" placeholder="Preferred Job Role" name="preferred_job_role"
                        value={formik.values.preferred_job_role} onChange={formik.handleChange} onBlur={formik.handleBlur}
              ...
              {/* ── Employee Fields ── */}
              {!formik.values.company_id && formik.values.student_type === "employee" && (
                <Grid item xs={12} md={6}>
                  <Stack sx={{ mt: 2, gap: 1 }}>
                    <FormLabel>Company Name*</FormLabel>
                    <TextField fullWidth id="company_name" name="company_name" placeholder="Company Name"
              ...
              {formik.values.student_type === "employee" && (
                <Grid item xs={12} md={6}>
                  <Stack sx={{ mt: 2, gap: 1 }}>
                    ...
                    <TextField fullWidth id="designation" placeholder="Designation" name="designation"
              ...
              {formik.values.student_type === "employee" && (
                <Grid item xs={12} md={6}>
                  <Stack sx={{ mt: 2, gap: 1 }}>
                    ...
                    <TextField fullWidth id="experience" placeholder="Experience" name="experience"
              ...
              {formik.values.student_type === "employee" && (
                <Grid item xs={12} md={6}>
                  <Stack sx={{ mt: 2, gap: 1 }}>
                    ...
                    <TextField fullWidth id="skills" placeholder="Skills" name="skills"
              ...
              {formik.values.student_type === "employee" && (
                <Grid item xs={12} md={6}>
                  <Stack sx={{ mt: 2, gap: 1 }}>
                    ...
                    <Autocomplete
                      id="company_id" options={companies || []}
                      getOptionLabel={(option) => option.company_name}
                      value={companies?.find((company) => company.company_id === formik.values.company_id) || null}
                      onChange={(event, newValue) => { formik.setFieldValue('company_id', newValue ? newValue.company_id : ''); }}
                      onBlur={formik.handleBlur}
                      renderInput={(params) => (
              ...
              <Grid item xs={12} md={6}>
                <Stack sx={{ mt: 2, gap: 1 }}>
                  ...
                  <TextField select fullWidth id="status" name="status"
              ...
              {/*Notes - show only when status changes*/}
              {shouldShowNotesField && (
                <Grid item xs={12}>
                  <Stack sx={{ mt: 2, gap: 1 }}>
                    <FormLabel>Notes*</FormLabel>
                    <TextField fullWidth id="notes" placeholder="Notes" name="notes" multiline rows={4}
            ...
            <DialogActions>
              <Button sx={{ backgroundColor: "#636E72", color: "#fff", "&:hover": { backgroundColor: "#636E72", color: "#fff" } }} onClick={handleClose}>Cancel</Button>
      ...
      <Dialog maxWidth="xs" TransitionComponent={PopupTransition} keepMounted fullWidth open={resetDialogOpen}
        onClose={(event, reason) => { if (reason !== 'backdropClick') handleClose(); }}
        BackdropProps={{ onClick: (event) => event.stopPropagation() }}
        sx={{ '& .MuiDialog-paper': { p: 0 }, transition: 'transform 225ms' }}>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <Stack spacing={2} sx={{ mt: 2 }}>
                ...
                <TextField autoFocus margin="dense" placeholder="New Password" type={showPassword ? 'text' : 'password'}
        ...
        <DialogActions>
          <Button sx={{ backgroundColor: "#636E72", color: "#fff", "&:hover": { backgroundColor: "#636E72", color: "#fff" } }} onClick={handleClose}>Cancel</Button>
          <Button onClick={handleResetSubmit} variant="contained" color="primary">Reset Password</Button>
        </DialogActions>
      </Dialog>

      {/* Notes Dialog */}
      <Dialog maxWidth="md" TransitionComponent={PopupTransition} keepMounted fullWidth open={notespopup}
        onClose={(event, reason) => { if (reason !== 'backdropClick') handleClose(); }}
        BackdropProps={{ onClick: (event) => event.stopPropagation() }}
        sx={{ '& .MuiDialog-paper': { p: 0 }, transition: 'transform 225ms' }}>
        <DialogTitle className="dialogTitle" sx={{ borderBottom: 1, borderColor: 'divider' }}>
            ...
            <Grid item xs={12}>
              <Stack spacing={3} sx={{ mt: 1 }}>
                {notes && notes.length > 0 ? (
                  notes.map((note) => (
                    <Box key={note.id} sx={{ p: 3, border: 1, borderColor: 'divider', borderRadius: 2, backgroundColor: 'background.paper', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', transition: 'all 0.2s ease-in-out', '&:hover': { boxShadow: '0 4px 12px rgba(0,0,0,0.15)', transform: 'translateY(-1px)' } }}>
                      <Grid container spacing={3} alignItems="flex-start">
                        <Grid item xs={12} sm={3}>
                          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                            <Typography variant="caption" color="text.secondary" fontWeight="bold">CREATED BY</Typography>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                              <Box sx={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: note.status === 'True' ? 'success.main' : 'error.main' }} />
                              <Typography variant="body1" fontWeight="medium" fontSize="1.1rem">{note.created_by || '-'}</Typography>
                            </Box>
                          </Box>
                        </Grid>
                        <Grid item xs={12} sm={4}>
                          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                            ...
                            <Box sx={{ p: 1.5, border: 1, borderColor: 'divider', borderRadius: 1, backgroundColor: 'grey.50', minHeight: '60px', display: 'flex', alignItems: 'flex-start' }}>
                              <Typography variant="body1" sx={{ lineHeight: 1.5, wordWrap: 'break-word', overflowWrap: 'break-word', whiteSpace: 'normal', wordBreak: 'break-word', width: '100%' }}>
                                {note.reason || '-'}
                              </Typography>
                            </Box>
                          </Box>
                        ...
                        <Grid item xs={12} sm={3}>
                          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                            ...
                            <Typography variant="body1" sx={{ backgroundColor: 'action.hover', px: 1.5, py: 0.5, borderRadius: 1, display: 'inline-block', width: 'fit-content' }}>
                        ...
                        <Grid item xs={12} sm={2}>
                          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                            ...
                            <Chip label={note.status === 'True' ? 'Active' : 'Inactive'} sx={{ color: note.status === 'True' ? 'success.dark' : 'error.dark', fontWeight: 600 }} />
                          </Box>
                        </Grid>
                      </Grid>
                    </Box>
                  ...
                  <Box sx={{ p: 6, textAlign: 'center', border: 2, borderColor: 'divider', borderStyle: 'dashed', borderRadius: 3, backgroundColor: 'action.hover', my: 2 }}>
                    <Typography variant="h6" color="text.secondary" gutterBottom>No Notes Available</Typography>
                    <Typography variant="body2" color="text.secondary">There are no notes to display at the moment.</Typography>
                  </Box>
                )}
              </Stack>
            </Grid>
