...
const StudentView = () => {
  ...
  // console.log(course,"course has been entered")
      ...
      {/* <Grid container spacing={3} sx={{ position: 'absolute', top: 18, left: '50%', zIndex: 1, width: '50%' }}>
        <Grid item xs={12} sx={{ justifyItems: 'flex-end' }}>
          <Stack sx={{ mb: { xs: -0.5, sm: 0.5 } }} spacing={1}>
            <IconButton variant="contained" color="white" size="medium" onClick={() => navigate(-1)} sx={{ width: 100, gap: 1,backgroundColor:"red" }}>
              <ArrowBack />
              Back
            </IconButton>
          </Stack>
                ...
                <TabPanel key={tab.id} value={value} index={index}>
                  ...
                  {tab.id === 'attendance' && (
                    <AddentencesTab
                      ...
                      course={course || profileData?.course_detail || profileData?.courses_assigned || []}
