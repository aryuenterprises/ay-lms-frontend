...
function PaymentReportsPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const studentPaymentSummaries = response.data.student_payment_summaries || [];
        const gatewayList            = response.data.gatway                 || [];

        // Autocomplete dropdown uses students list (has student_id + student_name)
        setStudents(students.map((s) => ({ student_id: s.student_id, student_name: s.student_name })));

        // Table rows use student_payment_summaries (has total_course_fee, paid_amount, remaining_amount, transactions)
        setAllStudentsData({ students, paymentDetails: studentPaymentSummaries });
  ...
  const getPaymentStatus = (student) => {
    const totalPaid   = student.paid_amount      || 0;
    const totalAmount = student.total_course_fee || 0;
    const remaining   = student.remaining_amount ?? (totalAmount - totalPaid);
  ...
  return (
    <MainCard sx={{ mt: 4 }}>
      <Grid sx={{ mb: 3, display: 'flex', justifyContent: 'space-between' }}>
        ...
        <Stack direction="column" spacing={1.5} sx={{ px: 2 }}>
          <Button variant="contained" sx={{ width: 100 }} onClick={handleOpenAddUserModal}>Add payment</Button>
        </Stack>
      </Grid>
                ...
                <Paper variant="outlined" sx={{ p: 3 }}>
                  ...
                  <Stack spacing={2}>
                    ...
                    <Box>
                      ...
                      <Typography variant="body1" color="warning.main" fontWeight="bold">{formatCurrency(selectedStudentDetails.remaining_amount ?? ((selectedStudentDetails.total_course_fee || 0) - (selectedStudentDetails.paid_amount || 0)))}</Typography>
                    </Box>
                    <Box>
          ...
          {selectedPayment && (
            <Stack spacing={3} sx={{ mt: 1 }}>
              <Box>
                ...
                <Typography variant="body2" color="textSecondary">Total Amount: {formatCurrency(selectedPayment?.total_course_fee)}</Typography>
                <Typography variant="body2" color="textSecondary">Paid Amount: {formatCurrency(selectedPayment?.paid_amount)}</Typography>
                <Typography variant="body2" color="textSecondary" sx={{ mt: 1 }}>
                  Remaining: {formatCurrency(selectedPayment?.remaining_amount ?? ((selectedPayment?.total_course_fee || 0) - (selectedPayment?.paid_amount || 0)))}
                </Typography>
              </Box>
