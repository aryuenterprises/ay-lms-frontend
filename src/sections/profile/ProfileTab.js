  ...
  return (
    <List component="nav" sx={{ p: 0, '& .MuiListItemIcon-root': { minWidth: 32, color: theme.palette.secondary.main } }}>
      ...
      {/* Show Student Information only if loginType is tutor */}
        ...
        <ListItemButton selected={selectedIndex === (loginType=="tutor" ? 1:3)} onClick={() => handleListItemClick(3, `/user/${loginType}`)}>
          <ListItemIcon>
            <Profile size={18} />
          </ListItemIcon>
          <ListItemText
           primary={ loginType==="tutor"?"student information":"tutor information"} 
      ...
      {loginType === 'student' && (
        ...
        <ListItemButton selected={selectedIndex === 3} onClick={() => handleListItemClick(3, '/user/student')}>
          ...
          <ListItemText primary="tutuor history" />
        </ListItemButton>
      )} */}
